    ...
    <div class="profile-container">
        ...
        <div class="page">
            ...
            <div class="page-content">
                ...
                $stmt = $mysqli->prepare("SELECT project_id,task,description,status,emp_id,proofs,start_date,end_date FROM `task_list` WHERE id = ?");
                $stmt->bind_param("i", $_GET['id']);
                $stmt->execute();
                $stmt->store_result();
                if ($stmt->num_rows > 0) {
                    $stmt->bind_result($project_id, $task, $description, $status, $emp_id, $proofs, $start_date, $end_date);
                    while ($stmt->fetch()) {
