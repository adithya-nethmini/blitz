<?php
    ...
    function addTask($name, $description, $priority, $deadline, $status){
        ...
        $stmt = $mysqli->prepare("INSERT INTO task(name, description, priority, deadline, status) VALUES(?,?,?,?,?)");
        $stmt->bind_param("sssss", $name, $description, $priority, $deadline, $status);
        $stmt->execute();
        if($stmt->affected_rows != 1){
            return "An error occurred. Please try again";
