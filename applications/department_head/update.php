...
<form class="form-inline" action="" method="post" autocomplete="off" >
        ...
        if($result){
            ...
            if($count_rows > 0){
                while($row = mysqli_fetch_assoc($result)){
                    ...
                    ?>
                    <option value="<?php echo $row['employeeid']."-".$row['name'] ?>" <?php echo isset($manager_id) && $manager_id == $row['employeeid']."-".$row['name'] ? 'selected' : '' ?> ><?php echo $employeeid ."". " - " .$name ."". " - " . $jobrole?></option>
                    <?php
                }
            }
        }
    ...
    <select required multiple="multiple" name="user_ids[]" id="user_ids[]" >
        ...
        while($row= $employees->fetch_assoc()){
            ...
            $jobrole = $row['jobrole'];
            ?>
            <option value="<?php echo  $row['name']  ?>"<?php echo isset($user_ids) && $user_ids == $row['name'] ? 'selected' : '' ?> >
